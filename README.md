...
1. **数据持久化**: 所有状态变更（工单通过/驳回/回查、出库对账/退货）均通过后端 API 写入 `backend/data.json`，刷新页面或重启服务数据不会丢失
2. 数据文件 `backend/data.json` 会自动创建，请勿手动删除；如需重置数据可删除该文件后重启后端服务
3. **端口说明**: 后端服务默认端口为 `8081`（原 8080 端口易被 Docker 等占用），已在 `vite.config.js` 中配置代理
4. Electron模式下需要同时启动后端服务，或配置打包后内置后端
5. 扫码功能需要配合硬件扫码枪使用，当前为预留接口
