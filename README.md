...
- **改圈服务**：戒指圈口调整
- **石重镶**：钻石/宝石加固重镶
- **合法状态迁移**：待确认 → 已确认 → 处理中 → 已完成 → 已取货
- **店长分配**：店长可指定任意售后处理人

### 3. 角色权限（严格隔离）
- **店长(manager)**：全权限，审批报价，分配保养，管理产品和客户
- **导购(salesperson)**：创建报价/预约，只能查看和操作自己的业务
- **售后专员(after_sales)**：处理分配给自己的保养预约，跟进维修进度
