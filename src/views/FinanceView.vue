  ...
  <div>
    ...
    <section class="section-card">
      <div class="card-header">
        ...
        <div>
          <el-radio-group
            :model-value="receiptTab"
            @change="receiptTab = $event"
            size="small"
          >
      ...
      <el-table :data="receiptFiltered" stripe style="width: 100%">
        ...
        <el-table-column label="关联申请" width="160">
          <template #default="{ row }">
            <span class="row-link" @click="jumpReturn(row.returnApplicationId)"
              >{{ row.returnApplicationId }}</span
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200">
          <template #default="{ row }">
            <el-button link type="primary" @click="openReceiptDetail(row)"
              >查看</el-button
            >
            <el-button
              link
              type="primary"
              @click="openReceiptEdit(row)"
              >补录</el-button
            >
            <el-button
    ...
    <section class="section-card">
      <div class="card-header">
        ...
        <div>
          <el-radio-group
            :model-value="reconTab"
            @change="reconTab = $event"
            size="small"
          >
            <el-radio-button value="all">全部</el-radio-button>
      ...
      <el-table :data="reconFiltered" stripe style="width: 100%">
        ...
        <el-table-column label="差额" width="80">
          <template #default="{ row }">
            <span
              :style="{
                color:
                  row.delta === 0 ? 'var(--app-success)' : 'var(--app-danger)',
              }"
              >{{ row.delta }}</span
            >
          </template>
        ...
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="reconTag(row.status)">{{
              reconLabel(row.status)
            }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160">
          <template #default="{ row }">
            <el-button link type="primary" @click="openReconDetail(row)"
              >查看</el-button
            >
            <el-button link type="primary" @click="openReconEdit(row)"
              >重新核对</el-button
            >
          </template>
        </el-table-column>
...
const receiptFiltered = computed(() => {
  ...
  if (receiptTab.value === "missing")
    return store.receipts.filter((r) => r.status === "missing")
  if (receiptTab.value === "confirmed")
    return store.receipts.filter((r) => r.status === "confirmed")
...
function reconTag(s: ReconciliationRecord["status"]) {
  return (
    (
      { matched: "success", mismatch: "danger", pending: "warning" } as const
    )[s] || ""
  )
}
...
function openReceiptDetail(row: SampleReceipt) {
...
function openReceiptEdit(row: SampleReceipt) {
  store.selectReceipt(row.id)
  store.openDrawer({
    visible: true,
    mode: "receipt",
...
function openReconDetail(row: ReconciliationRecord) {
...
function openReconEdit(row: ReconciliationRecord) {
  store.selectReconciliation(row.id)
  store.openDrawer({
    visible: true,
    mode: "reconcile",
