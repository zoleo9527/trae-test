...
import type { OrderStatus, SplitStatus, RefundStatus } from '../types'
...
export const Dashboard: React.FC = () => {
    ...
    splits.forEach((split) => {
      if (split.status === 'pending') {
        ...
        pendingItems.push({
          ...
          type: 'split',
          title: split.splitNo,
          subTitle: `待发货 ${split.items.length}类商品`,
  ...
  }, [orders, splits, receipts, refunds])
  ...
  const getItemLink = (item: typeof stats.pendingItems[0]) => {
    switch (item.type) {
      case 'order':
  ...
  const quickActions = [
    { label: '新建订单', action: () => {}, permission: permissions.canCreateOrder, icon: '📦' },
    { label: '拆单发货', action: () => {}, permission: permissions.canSplitOrder, icon: '🚚' },
    { label: '录入回执', action: () => {}, permission: permissions.canEnterReceipt, icon: '✅' },
    { label: '退款申请', action: () => {}, permission: permissions.canCreateRefund, icon: '💰' },
  ]

  return (
    <Layout
      title="仪表盘"
      subtitle={`当前角色：${currentRole === 'merchandiser' ? '跟单员' : currentRole === 'warehouse' ? '仓管员' : currentRole === 'finance' ? '财务员' : '管理层'}`}
    >
                ...
                {stats.pendingItems.length > 0 ? (
                  stats.pendingItems.map((item) => (
                    <Link
                      ...
                      to={getItemLink(item)}
                      className="flex items-center justify-between px-6 py-4 hover:bg-dark-50 transition-colors"
                    >
