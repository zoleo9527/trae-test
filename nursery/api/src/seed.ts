...
async function seed() {
  ...
  const diseaseRepo = AppDataSource.getRepository(Disease);
  const timelineRepo = AppDataSource.getRepository(DiseaseTimeline);
  ...
  const zhaoJianguo = userRepo.create({ name: '赵建国', role: UserRole.BASE_MANAGER, phone: '13800000001' });
  const zhangMing = userRepo.create({ name: '张明', role: UserRole.INSPECTOR, phone: '13800000002' });
  const liFang = userRepo.create({ name: '李芳', role: UserRole.INSPECTOR, phone: '13800000003' });
  const wangLei = userRepo.create({ name: '王磊', role: UserRole.SALES, phone: '13800000004' });
  const chenXiaohong = userRepo.create({ name: '陈小红', role: UserRole.SALES, phone: '13800000005' });
  ...
  const insp1 = inspectionRepo.create({
    ...
    remark: '苗木长势良好，无异常', status: InspectionStatus.COMPLETED,
  ...
  const insp2 = inspectionRepo.create({
    ...
    remark: '发现少量蚜虫', status: InspectionStatus.COMPLETED,
  ...
  const insp3a = inspectionRepo.create({
    ...
    growthStatus: '良好', soilCondition: '正常', moistureCondition: '适宜',
    remark: '苗木长势正常', status: InspectionStatus.COMPLETED,
  ...
  const insp3b = inspectionRepo.create({
    ...
    remark: '发现根腐病症状，部分苗木叶片发黄', status: InspectionStatus.COMPLETED,
  ...
  const insp4 = inspectionRepo.create({
    ...
    remark: '发现叶斑病', status: InspectionStatus.COMPLETED,
  ...
  const insp6 = inspectionRepo.create({
    ...
    remark: '发现介壳虫，危害严重，部分枝条枯死', status: InspectionStatus.COMPLETED,
  ...
  const insp7 = inspectionRepo.create({
    ...
    growthStatus: '良好', soilCondition: '正常', moistureCondition: '适宜',
    remark: '无异常，花期正常', status: InspectionStatus.COMPLETED,
  ...
  const insp8 = inspectionRepo.create({
    plotId: p1.id, inspectorId: u2.id,
    growthStatus: '良好', soilCondition: '正常', moistureCondition: '适宜',
    remark: '定期巡查，苗木正常', status: InspectionStatus.COMPLETED,
  ...
  const insp9 = inspectionRepo.create({
    plotId: p6.id, inspectorId: u3.id,
    growthStatus: '良好', soilCondition: '正常', moistureCondition: '适宜',
    remark: '', status: InspectionStatus.PENDING,
  ...
  const disease2 = diseaseRepo.create({
    ...
    type: '蚜虫', severity: DiseaseSeverity.MINOR,
    description: '银杏叶片背面发现少量蚜虫聚集，约5棵受影响',
    affectedQuantity: 5, status: DiseaseStatus.RESOLVED,
  ...
  const disease3b = diseaseRepo.create({
    ...
    type: '根腐病', severity: DiseaseSeverity.MAJOR,
    description: '桂花根部腐烂，50棵受影响，部分苗木叶片发黄枯萎，疑似排水不畅导致',
    affectedQuantity: 50, status: DiseaseStatus.CONFIRMED,
  ...
  const disease4 = diseaseRepo.create({
    ...
    type: '叶斑病', severity: DiseaseSeverity.MODERATE,
    description: '红叶石楠叶片出现褐色斑点，约20棵受影响，已有17天未处理',
    affectedQuantity: 20, status: DiseaseStatus.REPORTED,
  ...
  const disease6 = diseaseRepo.create({
    ...
    type: '介壳虫', severity: DiseaseSeverity.MAJOR,
    description: '广玉兰发现大量介壳虫，30棵受影响，枝条上布满白色蜡质分泌物，部分叶片发黄脱落',
    affectedQuantity: 30, status: DiseaseStatus.TREATING,
