import { a as toArray, i as isCSS, o as uniq, r as hashId, t as writeManifest } from "./manifest.mjs";
...
async function transformRequest(opts, id) {
	if (id && id.startsWith("/@id/__x00__")) id = "\0" + id.slice(12);
	if (id && id.startsWith("/@id/")) id = id.slice(5);
	if (id && !id.startsWith("/@fs/") && id.startsWith("/")) {
		const resolvedPath = resolve(opts.viteServer.config.root, "." + id);
		if (existsSync(resolvedPath)) id = resolvedPath;
	}
	id = id.replace(/^\/?(?=\w:)/, "/@fs/");
	const externalId = id.replace(/\?v=\w+$|^\/@fs/, "");
	if (await opts.isExternal(externalId)) return {
		code: `(global, module, _, exports, importMeta, ssrImport, ssrDynamicImport, ssrExportAll) =>
${genDynamicImport(builtinModules.includes(externalId.split("node:").pop()) ? externalId : isAbsolute(externalId) ? pathToFileURL(externalId).href : externalId, { wrapper: false })}
  .then(r => {
  ...
  .catch(e => {
	...
	return {
		code: `async function (global, module, exports, __vite_ssr_exports__, __vite_ssr_import_meta__, __vite_ssr_import__, __vite_ssr_dynamic_import__, __vite_ssr_exportAll__) {
${res.code || "/* empty */"};
}`,
		deps: res.deps || [],
...
async function transformRequestRecursive(opts, id, parent = "<entry>", chunks = {}) {
	...
	for (const dep of deps) await transformRequestRecursive(opts, dep, id, chunks);
...
`).join("\n"),
			`const __modules__ = ${genObjectFromRawEntries(chunks.map((chunk) => [chunk.id, hashId(chunk.id + "-" + chunk.code)]))}`,
			`
...
async function initViteDevBundler(ctx, onBuild) {
	...
	const _doBuild = async () => {
		...
		const manifestIds = [];
		for (const i of ids) if (isCSS(i)) manifestIds.push(i.slice(1));
