...
function resolveEslintClass(useFlatConfig, majorVersion) {
  if (majorVersion >= 10) {
    return ESLint;
  }
  if (useFlatConfig) {
  ...
  throw new Error(
    "Legacy eslintrc mode (useFlatConfig: false) requires ESLint v9. ESLint v10+ only supports flat config. Please upgrade your config to flat config or set useFlatConfig to true."
  );
}
  ...
  return {
    ...
    async configureServer({ root }) {
      ...
      if (!useFlatConfig && majorVersion >= 10) {
        console.warn(
          "[vite-plugin-checker] useFlatConfig: false is not supported with ESLint v10+. Falling back to flat config mode."
        );
      }
