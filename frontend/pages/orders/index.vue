<template>
  <div class="ws-container">
    <section class="ws-stats" v-if="stats.order_total">
      <div class="ws-stat-card">
        <div class="label">订单总数</div>
        <div class="value">{{ stats.order_total }}</div>
      </div>
      <div class="ws-stat-card pending" @click="selectedFocus = 'pending'">
        <div class="label">待复核</div>
        <div class="value">{{ stats.review_pending }}</div>
      </div>
      <div class="ws-stat-card rejected" @click="selectedFocus = 'rejected'">
        <div class="label">已驳回</div>
        <div class="value">{{ stats.review_rejected }}</div>
      </div>
      <div class="ws-stat-card recheck" @click="selectedFocus = 'recheck'">
    ...
    <section class="ws-section">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <h2>订单列表</h2>
        <div>
          <el-button v-if="selectedFocus" @click="selectedFocus = ''">清除过滤</el-button>
...
const loadStats = async () => {
  try { stats.value = await api.get<DashboardStats>('/dashboard') } catch (e) { /* ignore */ }
}
