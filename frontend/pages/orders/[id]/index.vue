<template>
  <div class="ws-container">
    ...
    <section class="ws-section">
      <div class="ws-order-head">
        <div>
          ...
          <div class="ws-order-sub">
            {{ order.studio_branch }} · 套餐：{{ order.package }} · 拍摄：{{ fmt(order.shoot_date) }} · 选片：{{ fmt(order.select_date) }}
          </div>
          <div class="ws-order-sub">
            店长：{{ order.store_manager }} · 选片：{{ order.selector }} · 修片：{{ order.retoucher }} · 客服：{{ order.customer_service }}
          </div>
        </div>
        <div class="ws-order-actions">
          <el-tag :type="order.balance_status === '已结清' ? 'success' : 'warning'" style="margin-right:8px">
            尾款：{{ order.balance_status }}
          </el-tag>
      ...
      <el-tabs v-model="activeTab" type="border-card">
        ...
        >
          <div class="ws-order-sub" style="margin-bottom:10px">
            回传时间：{{ fmt(b.delivered_at) }} · 备注：{{ b.remark || '-' }}
          </div>
    ...
    <el-dialog
      ...
      :title="photoDialog.photo ? photoDialog.photo.photo_name : ''"
      width="680px"
    >
      <div v-if="photoDialog.photo">
        ...
        <div style="margin-top:10px">
          ...
          <div><b>状态：</b>
            <span class="ws-badge" :class="photoDialog.photo.review_status">
              {{ photoDialog.photo.review_status }}
            </span>
          </div>
        ...
        <el-form label-width="80px">
          ...
          <el-form-item label="反馈">
            <el-input v-model="reviewForm.feedback" type="textarea" :rows="3" placeholder="针对这张图的具体反馈..." />
          </el-form-item>
    ...
    <el-dialog v-model="addBatchDialog.visible" title="新增回传批次" width="520px">
      <el-form label-width="90px">
        <el-form-item label="批次序号">
          <el-input-number v-model="addBatchForm.batch_no" :min="1" />
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="addBatchForm.remark" type="textarea" :rows="2" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const photoDialog = reactive<{ visible: boolean; photo: Photo | null; batch: Batch | null }>({
...
const submitReview = async () => {
  ...
  try {
    ...
    photoDialog.photo.reviews = [...(photoDialog.photo.reviews || []), {
      id: -1,
      reviewer: reviewForm.reviewer,
      verdict: reviewForm.verdict,
      feedback: reviewForm.feedback,
      version_at_review: updated.version,
