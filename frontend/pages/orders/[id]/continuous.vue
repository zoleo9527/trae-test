  ...
  <div class="ws-container" v-loading="loading">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
        <div>
          ...
          <div class="ws-order-sub" style="margin-top:4px">
            ...
            <el-tag size="small" :type="continuous.order.balance_status === '已结清' ? 'success' : 'warning'" style="margin-left:4px">
              尾款：{{ continuous.order.balance_status }}
            </el-tag>
          </div>
        </div>
        <div>
    ...
    <section class="ws-section" v-if="continuous.batches && continuous.batches.length">
      ...
      <el-alert
        type="info"
        :closable="false"
        title="同一订单所有回传批次在这里聚合，避免翻聊天记录拼接上下文。点击任意照片可复核、驳回或回查。"
        style="margin-bottom:12px"
      />

      <el-tabs v-model="activeTab" type="border-card">
        <el-tab-pane
          v-for="b in continuous.batches"
          :key="b.batch_id"
          :label="`第 ${b.batch_no} 次 · ${b.status}`"
          :name="String(b.batch_id)"
        >
          <div class="ws-order-sub" style="margin-bottom:10px">
            回传：{{ fmt(b.delivered_at) }} · 备注：{{ b.remark || '-' }}
          </div>
      ...
      <div class="ws-compare">
        <div v-for="row in comparisonRows" :key="row.key" class="ws-compare-item">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
            <div v-for="(p, idx) in row.photos" :key="idx" style="background:#fff;border:1px solid #eee;border-radius:6px;overflow:hidden;">
              <img :src="p.image_url" :alt="p.photo_name" />
    ...
    <el-dialog v-model="photoDialog.visible" :title="photoDialog.photo ? photoDialog.photo.photo_name : ''" width="680px">
      <div v-if="photoDialog.photo">
        <img
          :src="photoDialog.photo.image_url"
          :alt="photoDialog.photo.photo_name"
          style="width:100%;max-height:360px;object-fit:cover;border-radius:6px;background:#eee"
        />
        <div style="margin-top:10px">
          <b>版本：</b>v{{ photoDialog.photo.version }} ·
          <b>类别：</b>{{ photoDialog.photo.category }} ·
          <b>状态：</b><span class="ws-badge" :class="photoDialog.photo.review_status">{{ photoDialog.photo.review_status }}</span>
        </div>
        <div v-if="photoDialog.photo.latest_feedback" style="margin-top:6px">
          <b>最新反馈：</b>{{ photoDialog.photo.latest_feedback }}
        </div>

        <el-divider>复核历史</el-divider>
        <el-timeline v-if="photoDialog.photo.reviews && photoDialog.photo.reviews.length">
          <el-timeline-item
            v-for="r in photoDialog.photo.reviews"
            :key="r.id"
          ...
          >
            <div>
              <b>{{ r.reviewer }}</b> 复核：
              <el-tag size="small" :type="r.verdict === '通过' ? 'success' : (r.verdict === '驳回' ? 'danger' : 'warning')">
                {{ r.verdict }}
              </el-tag>
              <span style="margin-left:8px;color:#888;font-size:12px">v{{ r.version_at_review }}</span>
            </div>
...
const photoDialog = reactive<{ visible: boolean; photo: Photo | null; batch: BatchGroup | null }>({
  visible: false, photo: null, batch: null
})
...
const comparisonRows = computed(() => {
  const map: Record<string, { key: string; status: string; photos: Photo[] }> = {}
  for (const b of continuous.batches) {
    for (const p of b.photos) {
      const key = `${p.category}`
      if (!map[key]) map[key] = { key, status: p.review_status, photos: [] }
      map[key].photos.push(p)
      map[key].status = p.review_status
    }
  }
...
const submitReview = async () => {
  ...
  try {
    const updated = await api.post<Photo>(`/photos/${photoDialog.photo.id}/review`, { ...reviewForm })
    Object.assign(photoDialog.photo, updated)
