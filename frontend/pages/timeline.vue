      ...
      <div v-else class="relative">
        ...
        <div v-for="(item, index) in timeline" :key="item.id" class="relative pl-16 pb-8 last:pb-0">
          ...
          <div class="bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-colors">
            ...
            <div v-else-if="item.entity_type === 'reception_field' || item.entity_type === 'settlement_field'" class="flex items-center space-x-2 mb-2">
              <span class="text-sm text-gray-600">原值:</span>
              <span class="text-sm font-medium text-gray-900 bg-gray-200 px-2 py-1 rounded">{{ item.old_status }}</span>
              <span class="text-gray-400">→</span>
              <span class="text-sm font-medium text-gray-900 bg-indigo-100 text-indigo-700 px-2 py-1 rounded">{{ item.new_status }}</span>
            </div>
