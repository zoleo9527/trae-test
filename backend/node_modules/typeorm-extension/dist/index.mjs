...
import { InstanceChecker, Brackets, DataSource, MigrationExecutor, Table, MssqlParameter, Repository } from 'typeorm';
import path from 'node:path';
import fs from 'node:fs';
import { consola } from 'consola';
import { toBool, oneOf, read, readInt, readBool, toArray, readArray } from 'envix';
...
function applyQueryParseOutput(query, context, options = {}) {
    if (context.fields) {
...
function applyQuery(query, input, options = {}) {
    ...
    const output = applyQueryParseOutput(query, parseQuery(input, options), options);
    return {
        ...output,
...
async function normalizeDataSourceOptions(context) {
    let options;
    if (context.options) {
        options = {
            ...context.options
...
async function buildDatabaseCreateContext(input = {}) {
    return {
        ...input,
...
async function buildDatabaseDropContext(input = {}) {
    return {
        ...input,
        options: await normalizeDataSourceOptions(input),
...
async function generateMigration(context) {
    ...
    if (!context.preview) {
        ...
        await fs.promises.writeFile(path.join(directoryPath, fileName), content, {
            encoding: 'utf-8'
        });
    }
    return {
...
async function synchronizeDatabaseSchema(input) {
    ...
    if (options.migrations) {
        migrationsCount = Array.isArray(options.migrations) ? options.migrations.length : Object.keys(options.migrations).length;
    }
    let migrations = [];
...
async function createPostgresDatabase(input = {}) {
...
async function createCockroachDBDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
    const connection = await createSimplePostgresConnection(driver, options, context);
...
async function dropCockroachDBDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
    const connection = await createSimplePostgresConnection(driver, options, context);
...
async function createMongoDBDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
...
async function dropMongoDBDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
    const client = await createSimpleMongoDBConnection(driver, options);
...
async function createMsSQLDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
...
async function dropMsSQLDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
    const connection = await createSimpleMsSQLConnection(driver, options);
...
async function createMySQLDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
...
async function dropMySQLDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
    const connection = await createSimpleMySQLConnection(driver, options);
...
async function createOracleDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    const driver = createDriver(context.options);
...
async function createSQLiteDatabase(input = {}) {
    const context = await buildDatabaseCreateContext(input);
    const options = buildDriverOptions(context.options);
    if (!options.database) {
...
async function dropSQLiteDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    const options = buildDriverOptions(context.options);
    if (!options.database) {
        throw OptionsError.databaseNotDefined();
 ...
 *
 * @param input
 */ async function dropDatabase(input = {}) {
    const context = await buildDatabaseDropContext(input);
    let output;
    switch(context.options.type){
        case 'mongodb':
 ...
 */ async function validateEntityJoinColumns(entity, options) {
    ...
    for(let i = 0; i < entityMetadata.relations.length; i++){
        ...
        for(let j = 0; j < relation.joinColumns.length; j++){
            ...
            if (joinColumn.isNullable && entity[joinColumn.propertyName] === null) {
                skipRelation = true;
                break;
            }
...
function applyWhereExpression(qb, data, type) {
    ...
    for(let i = 0; i < keys.length; i++){
        elements.push({
            ...
            operator: type === 'target' ? FilterComparisonOperator.EQUAL : FilterComparisonOperator.NOT_EQUAL
        });
    }
 ...
 */ async function isEntityUnique(options) {
    ...
    const columnGroups = [];
    if (metadata.ownUniques && metadata.ownUniques.length > 0) {
        for(let i = 0; i < metadata.ownUniques.length; i++){
            columnGroups.push(metadata.ownUniques[i].columns.map((column)=>column.propertyName));
        }
    } else {
        for(let i = 0; i < metadata.indices.length; i++){
            ...
            columnGroups.push(index.columns.map((column)=>column.propertyName));
        }
    }
